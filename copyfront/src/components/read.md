...
const DeanDashboard = () => {
  ...
  const checkAuthentication = async () => {
    try {
      ...
      const deanDataString = localStorage.getItem('dean');
      const token = localStorage.getItem('token');
      
      console.log('Dean Data from storage:', deanDataString);
      console.log('Token from storage:', token);

      if (!deanDataString || deanDataString === 'undefined' || !token) {
        console.log('❌ No Dean data found, redirecting to login');
        navigate('/login/dean');
        return;
      }

      const deanData = JSON.parse(deanDataString);
      
      if (deanData.role !== 'dean') {
        console.log('❌ User is not Dean, redirecting');
        navigate('/login/dean');
        return;
      }
      
      console.log('✅ Dean authenticated:', deanData);
      setDean(deanData);
      
      await fetchDeanData();

    } catch (error) {
      console.error('❌ Error in Dean authentication:', error);
      handleAutoLogout();
    }
  };
